...
public class Controller : MonoBehaviour, IPlayer
{
    ...
    [SerializeField] private GameObject ObjetoALanzar;
    ...
    public void Crouching ()
    {
        if (Input.GetKeyDown(KeyCode.LeftControl))
        {
            ...
            {
                if (Physics.Raycast(m_raycastOrigin.position, Vector3.up, 0.01f, 1))
                {
                    Debug.Log("Hit");
                    Debug.Log(hitInfo.transform.gameObject.name);
                }

                else if (!Physics.Raycast(m_raycastOrigin.position, Vector3.up, 0.01f, 1))
                {
                    isCrouched = false;
                    m_charCtrl.height = 2;
    ...
    public void Attack()
    {
        if (Input.GetKeyDown(KeyCode.Q) & !hasJumped)
        {
            ...
            attackHitInfo =  Physics.SphereCastAll(this.transform.position, attackDistance, Vector3.forward, AttackRaycastLayers);
            for(int i = 0; i <= attackHitInfo.Length - 1; i++)
            {
                Debug.Log("for");
                if (attackHitInfo[i].collider.GetComponent<IEnemy>() != null)
                {
                    attackHitInfo[i].collider.GetComponent<IEnemy>().IEnemyDamage();
                    Debug.Log("dentro");
                }
            }
        }     
    ...
    public void IPlayerAction(Controller Player)
    {
        SceneManager.LoadScene("w");
    }
    public void Lanzamiento()
    {
        GameObject Piedra = Instantiate(ObjetoALanzar, transform.position + transform.forward * 2 + transform.up, transform.rotation);
    }
